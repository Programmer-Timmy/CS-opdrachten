...
namespace NatSim
{
    ...
    {
        public Vingerhoedskruid() : base(1, "Gramineae", 4, Bloeiwijze.aar)
        {
            initClass(new Point(0, 0));
        }
        public Vingerhoedskruid(Point locatie) : base(1, "Gramineae", 4, Bloeiwijze.aar)
        {
            initClass(locatie);
        }
        ...
        public static void Zaaien(Point locatie, Graphics papier, int lengte, int breedte, int zaaiAfstand)
        {
            ...
            while (puntX < lengte)
            {
                while (puntY < breedte)
                {
                    ...
                    Vingerhoedskruid gras = new Vingerhoedskruid(locatie);
                    gras.Teken(papier);
